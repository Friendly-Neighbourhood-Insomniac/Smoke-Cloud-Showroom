...
class PlayerController {
   ...
   * @param {number} cameraRotation The current horizontal rotation (yaw) of the camera.
   */
  update(deltaTime, cameraRotation) {
    ...
    // The cameraRotation parameter is the yaw of the camera.
...
class FirstPersonCameraController {
  ...
  setupMouseControls() {
    ...
    this.boundMouseMove = this.onMouseMove.bind(this); // Store the bound function
  ...
  enable() {
    this.enabled = true;
    this.enabled = true;
    // rotationY is typically set by the game logic when switching to FPC
    // this.rotationX = 0; // Reset pitch on enable, or preserve previous pitch if preferred
    this.hidePlayer(); // Ensure player model is hidden
    // Pointer lock is requested by game logic (e.g., on startExperience or info panel close)
  }
  disable() {
    this.enabled = false;
    this.showPlayer(); // Ensure player model is visible
    // Pointer lock is released by game logic (e.g., on info panel open)
  ...
  update() {
    if (!this.enabled) return 0;
    // Set player rotation to match camera's horizontal rotation (only if pointer is locked)
    // If pointer is not locked (e.g. info panel is open), we don't want mouse to control player/camera view
    // The game loop will handle this logic by not calling update or by passing stale cameraYaw
